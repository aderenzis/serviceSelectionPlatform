...
//			panelOpciones.add(getJTipoWrapper(), pogrid2);
	...
	private JTextField getJwtpPath() {
		if (jwtpPath == null) {
			...
			String wsdlPath = Configuration.getInstance().getWsdlsPath()+File.separator+"ServiceTomcat"+File.separator+"SimpleCalculatorService.wsdl";
			jwtpPath.setText(wsdlPath);
...
//	}
	...
	protected void classPath(JTextField campo) {
		...
		fc.setCurrentDirectory(new File(campo.getText()));
	...
	public void generaCliente( String[] args ) throws IOException
    {
		...
		ProcessBuilder pb;
		pb = new ProcessBuilder(System.getenv("AXIS2_HOME")+"/bin/wsdl2java.sh","-o",pathtoDeploy+"/stub","-uri",pathToWSDL);
//		pb = new ProcessBuilder(System.getenv("AXIS2_HOME")+File.separator+"bin"+File.separator+"wsdl2java.sh","-o",pathtoDeploy+File.separator+"stub","-uri",pathToWSDL);
//		pb = new (System.getenv("AXIS2_HOME")+"\\bin\\wsdl2java.bat","-o",pathToWSDL.substring(0, pathToWSDL.indexOf(".wsdl"))+"\\temp\\stub","-uri",pathToWSDL);
//		if (packageName.equals(""))//cableado
//			 pb = new ("D:\\Dropbox\\TESIS\\axis2-1.6.2\\bin\\wsdl2java.bat","-o",pathtoDeploy,"-uri",pathToWSDL);
	...
	private JButton getJbBuildFile() {
		if (jbBuildFile == null) {
			...
			jbBuildFile.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					String proxyPath = Configuration.getInstance().getWorkingPath()+"temp/proxy";
					String args[]={proxyPath,jwtpPath.getText(),""};
					try {
						logError("jwtpPath.toString(): "+jwtpPath.getText());
						generaCliente(args);
					} catch (IOException e1) {
...
//						if ((nombre.equals("For Services")) || (nombre.equals("For Components"))){
							ich.buildWrappers(0,0,"S",proxyPath,jwtpPath.getText());
